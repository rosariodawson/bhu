...
class DBManager:
    ...
    async def query_recent_events():
        cutoff = int(
            (
                datetime.now(timezone.utc) - timedelta(hours=REPLAY_WINDOW_HOURS)
            ).timestamp()
        )
        async with aiosqlite.connect(DB_FILE) as db:
        ...
        try:
            async with self.session.get(url, params=params, timeout=20) as response:
                if response.status in [429, 418]:
                    ...
                    logging.warning(
                        f"[BINANCE] Rate limited. Halting requests for {retry_after}s."
                    )
...
class StrategyEngine:
    ...
    async def run_universe_curation(self, symbols: List[str]) -> List[str]:
        ...
        try:
            try:
                ...
                logging.error(
                    f"Failed to fetch bulk data during gather: {e}", exc_info=True
                )
                raise
        ...
        try:
            kl_spot_d, kl_spot_5m, oi_hist_5m, kl_perp_1m = await asyncio.gather(
                ...
                self.mgr.get_klines(
                    symbol,
                    "5m",
                    REPLAY_WINDOW_HOURS * 12 + L2_MOMENTUM_IGNITION_LOOKBACK,
                    is_f=False,
                ),
                self.mgr.get_oi_history(symbol, "5m", REPLAY_WINDOW_HOURS * 12 + 2),
                self.mgr.get_klines(symbol, "1m", 1, is_f=True),
            )
            if not all([kl_spot_d, kl_spot_5m, oi_hist_5m, kl_perp_1m]):
                return
            ...
            for i in range(L2_MOMENTUM_IGNITION_LOOKBACK, len(df_spot_5m)):
                event_time = pd.to_datetime(
                    df_spot_5m.iloc[i]["ts"], unit="ms", utc=True
                )

                spot_slice = df_spot_5m.iloc[
                    i - L2_MOMENTUM_IGNITION_LOOKBACK : i
                ].values.tolist()
                oi_slice = (
                    df_oi_5m[
                        pd.to_datetime(df_oi_5m["timestamp"], unit="ms", utc=True)
                        <= event_time
                    ]
                    .tail(2)
                    .to_dict("records")
                ...
                if ignition_score and ignition_score >= L2_MIN_MOMENTUM_IGNITION_SCORE:
                    metrics = {
                        "l1_cvd_pct": round(spot_accum_pct, 2),
                        "l2_ign_score": round(ignition_score, 2),
                    }
    ...
    def _calculate_cvd_from_klines(self, kl: List) -> float:
        if not kl:
            return 0.0
        deltas = [
            (float(k[5]) if float(k[4]) > float(k[1]) else -float(k[5])) for k in kl
        ]
    ...
    def _calculate_momentum_ignition(
        self, spot_klines_5m: List, oi_hist_5m: List
    ) -> Optional[float]:
        if len(oi_hist_5m) < 2:
            return None
        ...
        oi_change_5m = (
            (
                float(oi_hist_5m[-1]["sumOpenInterest"])
                / float(oi_hist_5m[-2]["sumOpenInterest"])
            )
            - 1
        ) * 100
...
class TradeManager:
    def __init__(
        ...
        log_msg = (
            f"TradeManager initialized: Risk={portfolio_risk_pct}%, R:R={reward_ratio}"
        )
    ...
    async def get_atr(self, symbol: str) -> Optional[float]:
        try:
            ...
            df["tr"] = np.maximum(
                df["h"] - df["l"],
                np.maximum(
                    abs(df["h"] - df["c"].shift()), abs(df["l"] - df["c"].shift())
                ),
            )
    ...
    async def execute_trade_entry(self, event_id: str, symbol: str):
        try:
            ...
            if not atr:
                logging.warning(
                    f"Skipping trade for {symbol}: Could not calculate ATR."
                )
                return
            ...
            if not self.LIVE_TRADING:
                ...
                if liquidity_clusters:
                    logging.info(
                        f"Liquidity: Bid Cluster @ {liquidity_clusters['bid_cluster']:.4f}, Ask Cluster @ {liquidity_clusters['ask_cluster']:.4f}"
                    )
                logging.info(f"----------------------")
                ...
                logging.info(
                    f"LIVE ENTRY FILLED: {position_size} {symbol} @ {actual_entry_price}"
                )
        ...
        async with aiohttp.ClientSession() as session:
            ...
            while True:
                try:
                    ...
                    logging.info(
                        f"--- Step 1 & 2: Running {REPLAY_WINDOW_HOURS}-Hour Replay Scan ---"
                    )
                    tasks = [
                        self.engine.process_symbol_replay(s) for s in curated_symbols
                    ]
                    await asyncio.gather(*tasks)

                    logging.info(
                        "--- Step 3: Ranking Events & Checking Execution Status ---"
                    )
                    ...
                    await self._check_and_process_executable_signals(
                        events, live_trades_from_db
                    )
    ...
    async def _check_and_process_executable_signals(self, events, live_trades_from_db):
        ...
        for event in detected_events:
            ...
            if has_microstructure_trigger:
                logging.info(
                    f"[TRADE] 🔥 L1/L2 SETUP CONFIRMED + MICROSTRUCTURE TRIGGER for {symbol}. Handing to TradeManager."
                )
...
"""
        ...
        for i, res in enumerate(top_10_deduped):
            ...
            entry_str, sl_str, tp1_str, tp2_str, tp3_str, meta_str = (
                "—",
                "—",
                "—",
                "—",
            ...
            if trade_data:
                ...
                tp3_color = (
                    f"{Colors.GREEN}{tp3_str}{Colors.END}"  # Light/Faded Green (TP3)
                )
                sl_color = (
                    f"{Colors.RED}{Colors.BOLD}{sl_str}{Colors.END}"  # Bold Red for SL
                )
            ...
            score_str = (
                f"{Colors.CYAN}{Colors.BOLD}{score:.0f}{Colors.END}"
                if score > 5000
                else f"{score:.0f}"
            )
