...
"""
Vertical Explosion Catcher v5.6 — Long-Term Multi-OI
---------------------------------------------------------------------------------
Scans Binance Futures for 24-hour momentum signals.

What’s new in this patch
- Correct Futures→Spot mapping
- 12-point signal scoring system based on 24H PRICE CHANGE.
- 2-HOUR OI SPIKE detection (20min vs 100min).
- NEW: Added Bybit and Bitget as fallback sources for 2-HOUR OI data.
...
def make_logger()->logging.Logger:
    logger = logging.getLogger("vec56")
...
async def get_price_change_percent(session: aiohttp.ClientSession, fut_symbol: str) -> Optional[float]:
...
async def get_oi_spike_percent_binance(session: aiohttp.ClientSession, fut_symbol: str) -> float:
    """
    Calculates the 2-hour OI spike from BINANCE.
    Compares the last 20 mins (4x 5m) vs the previous 100 mins (20x 5m).
    """
    # Fetch last 24 5-min periods (2 hours)
    ...
    try:
        # Baseline: 100 mins (20 periods) before the last 20 mins
        baseline_data = [safe_float(d.get("sumOpenInterestValue")) for d in js[0:20]]
        # Recent: Last 20 mins (4 periods)
        ...
        return max(0.0, spike_pct) # Only return positive spikes
    except Exception as e:
...
async def get_oi_spike_percent_bybit(session: aiohttp.ClientSession, fut_symbol: str) -> float:
    """
    Calculates the 2-hour OI spike from BYBIT.
    Compares the last 20 mins (4x 5m) vs the previous 100 mins (20x 5m).
    """
    ...
    try:
        ...
        data_list.reverse()
        baseline_data = [safe_float(d.get("openInterest")) for d in data_list[0:20]]
...
async def get_oi_spike_percent_bitget(session: aiohttp.ClientSession, fut_symbol: str) -> float:
    """
    Calculates the 2-hour OI spike from BITGET.
    Compares the last 20 mins (4x 5m) vs the previous 100 mins (20x 5m).
    ...
    params = {"symbol": bitget_symbol, "interval": "5m", "limit": 24}
...
def evaluate_m1_score(
    ...
    # 1. Price Change (Max: 4 points)
    if price_chg_pct is not None:
        if price_chg_pct > 0:
            score += 1.0
            synopsis_notes.append("Δ>0")
        if price_chg_pct > 5.0:
            score += 2.0 # Extra points for strong change
            synopsis_notes.append("Δ>5%")
        if price_chg_pct < 50.0:
            score += 1.0 # Not *too* high (not chasing a 100% pump)
...
async def scan_symbol(
    ...
    # Gather data
    price_chg = await get_price_change_percent(session, fut_symbol) # 24h Price Change
    ...
    # 1. Run M1 Scoring
    m1_score, synopsis = evaluate_m1_score(price_chg, spot_taker, fut_taker, avg_funding_raw, oi_spike_pct)
    ...
    # Log the pass
    log.info("[+] SCORE %.1f: %s (PriceChg: %s%%, FutTaker: %s, Fund: %s, OI Spike: %.1f%% [Source: %s])",
             m1_score,
             fut_symbol,
             f"{price_chg:.2f}" if price_chg is not None else "—",
    ...
    # Note: day_change must be converted to decimal (5.0% -> 0.05)
    result = {
        "symbol": fut_symbol,
        "bias": "long",
        "m1_score": m1_score,
        "day_change": price_chg / 100.0 if price_chg is not None else None,
...
async def main_async(args: List[str]) -> None:
    import argparse
    ap = argparse.ArgumentParser(description="Vertical Explosion Catcher v5.6 — Long-Term Multi-OI")
    ...
    log.info("--- Vertical Explosion Catcher v5.6 (Long-Term Multi-OI) | %s ---", now_iso_utc())
